...
import {ConversationMessageFilter} from 'react-native-truesight-chat';
import type {DocumentPickerResponse} from 'react-native-document-picker';
import DocumentPicker, {isInProgress} from 'react-native-document-picker';
import type {ConversationMessageReducer} from '../reducers/conversation-message-reducer';
import {
...
import {conversationMessageRepository} from 'src/repositories/conversation-message-repository';
...
export function useChat(
  ...
  const [
    ...
    conversationMessageRepository.list,
    conversationMessageRepository.count,
    SearchField.NAME,
    SearchType.CONTAIN,
    {
  ...
  const handleChooseImage = React.useCallback(
    (
      ...
      subscription.add(
        conversationMessageRepository.multiUploadFile(images).subscribe({
          next: (results: ConversationFile[]) => {
            if (results.length > 0) {
              const conversationAttachments = results.map(image => ({
                name: image.name,
                url: server.serverUrl + image?.path,
